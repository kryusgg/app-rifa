  ...
  const handleRequestReservation = async (e) => {
    ...
    try {
      ...
      if (receiptFile) {
        ...
        if (result.success) {
          uploadedReceiptUrl = result.data.url; 
        } else {
  ...
  return (
    ...
    >
      ...
      <div className="max-w-6xl mx-auto space-y-6 relative z-10">
        ...
        {/* CABEÇALHO */}
        <div className="bg-white/95 backdrop-blur-sm p-6 rounded-2xl shadow-lg border border-white/20 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div className="flex-1 w-full">
            ...
            <div className="w-full max-w-md bg-gray-200 rounded-full h-3 overflow-hidden border border-gray-300 flex shadow-inner">
            ...
            <p className="text-xs text-gray-600 mt-2 font-medium">{progressPercent.toFixed(1)}% reservado/vendido</p>
          </div>
          <div className="flex bg-gray-100 p-1 rounded-lg w-full sm:w-auto justify-center shadow-sm">
        ...
        {activeTab === 'grid' && (
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
            <div className="lg:col-span-1">
              <div className="bg-white/95 backdrop-blur-sm p-6 rounded-2xl shadow-lg border border-white/20 sticky top-6">
                ...
                <form onSubmit={handleRequestReservation} className="space-y-4">
                  ...
                  <div className="border-2 border-dashed border-gray-300 rounded-xl p-4 text-center hover:bg-gray-50 transition-colors bg-white">
                    ...
                    <label htmlFor="receipt-upload" className="cursor-pointer flex flex-col items-center gap-2">
                      ...
                      <span className="text-xs text-gray-500">Opcional. Apenas imagens.</span>
                    </label>
                  </div>

                  {overlapError && <div className="bg-red-50 text-red-700 p-3 rounded-lg text-sm flex gap-2"><AlertCircle size={16} />{overlapError}</div>}

                  <button type="submit" disabled={!name.trim() || selectedNumbers.length === 0 || overlapError || isUploading} className="w-full bg-blue-600 hover:bg-blue-700 disabled:bg-gray-400 text-white font-medium py-3 rounded-lg flex items-center justify-center gap-2 transition-colors shadow-md">
            ...
            <div className="lg:col-span-3 bg-white/95 backdrop-blur-sm p-6 rounded-2xl shadow-lg border border-white/20 overflow-hidden">
              <div className="flex items-center gap-4 mb-4 text-xs font-medium text-gray-700 justify-center sm:justify-start">
                ...
                <div className="flex items-center gap-1"><span className="w-3 h-3 rounded-full bg-yellow-100 border border-yellow-300 shadow-sm"></span> Reservado</div>
                <div className="flex items-center gap-1"><span className="w-3 h-3 rounded-full bg-red-100 border border-red-300 shadow-sm"></span> Vendido</div>
              </div>
            ...
            {!isAuthenticated ? (
              ...
              <div>
                <div className="flex justify-between items-center mb-8 pb-4 border-b border-gray-200">
                  ...
                  <button onClick={() => setIsAuthenticated(false)} className="text-sm text-gray-500 hover:text-red-500 font-medium">Sair do Painel</button>
                </div>
                ...
                <div className="mb-10">
                  ...
                  {Object.keys(pendingGroups).length === 0 ? (
                    <p className="text-gray-500 text-sm italic bg-white p-4 rounded-lg border border-dashed border-gray-300">Nenhuma reserva pendente.</p>
                  ) : (
                    <div className="space-y-3">
                      {Object.entries(pendingGroups).map(([owner, data]) => (
                        <div key={`pend-${owner}`} className="border border-yellow-200 rounded-xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-yellow-50/80 shadow-sm">
                          <div>
                            <h4 className="font-bold text-gray-800 flex items-center gap-2"><User size={16}/> {owner}</h4>
                            
                            {/* Link do Comprovante (ImgBB) */}
                            {data.receiptUrl && (
                              <a href={data.receiptUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-sm text-blue-600 hover:text-blue-800 mt-2 bg-blue-50 px-3 py-1 rounded-full font-medium transition-colors border border-blue-100 shadow-sm">
                            ...
                            <div className="flex flex-wrap gap-2 mt-3">
                              {data.nums.sort((a, b) => a - b).map(n => (
                                <div key={n} className="inline-flex items-center gap-1 bg-white border border-yellow-300 px-2 py-1 rounded text-sm font-medium text-yellow-700 group shadow-sm">
                                  <span>#{n}</span>
                          ...
                          <div className="flex gap-2 shrink-0">
                            <button onClick={() => handleApprovePerson(data.nums)} className="flex items-center gap-1 px-3 py-2 text-sm font-medium text-white bg-emerald-500 rounded-lg hover:bg-emerald-600 transition-colors shadow-sm"><Check size={16} /> Confirmar</button>
                            <button onClick={() => handleDeletePerson(owner, data.nums)} className="flex items-center gap-1 px-3 py-2 text-sm font-medium text-red-600 bg-white border border-red-200 rounded-lg hover:bg-red-50 transition-colors shadow-sm"><Trash2 size={16} /> Excluir</button>
                          </div>
                        </div>
                      ))}
                    </div>
                ...
                <div>
                  ...
                  {Object.keys(approvedGroups).length === 0 ? (
                    <p className="text-gray-500 text-sm italic bg-white p-4 rounded-lg border border-dashed border-gray-300">Nenhum número confirmado ainda.</p>
                  ) : (
                    <div className="space-y-3">
                      {Object.entries(approvedGroups).map(([owner, data]) => (
                        <div key={`appr-${owner}`} className="border border-gray-200 rounded-xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white shadow-sm">
                          <div>
                            <h4 className="font-bold text-gray-800 flex items-center gap-2"><User size={16}/> {owner}</h4>
                            <div className="flex flex-wrap gap-2 mt-2">
                              {data.nums.sort((a, b) => a - b).map(n => (
                                <div key={n} className="inline-flex items-center gap-1 bg-gray-50 border border-gray-200 px-2 py-1 rounded text-sm font-medium text-gray-600 group shadow-sm">
                                  <span>#{n}</span>
                                  <button onClick={() => handleDeleteSingle(n)} className="text-gray-400 hover:text-red-500 ml-1 opacity-0 group-hover:opacity-100"><Trash2 size={14} /></button>
                                </div>
                              ))}
                          ...
                          <button onClick={() => handleDeletePerson(owner, data.nums)} className="flex items-center gap-1 px-3 py-2 text-sm font-medium text-gray-500 bg-white border border-gray-200 rounded-lg hover:text-red-600 hover:border-red-200 hover:bg-red-50 transition-colors shrink-0 shadow-sm"><Trash2 size={16} /> Cancelar Venda</button>
                        </div>
                      ))}
                    </div>
                  )}
